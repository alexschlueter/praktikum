...
\subsection{Magnetfeld in horizontaler Symmetrieebene einer kurzen Spule}
\begin{figure}[H]
  \centering
\input{aufgabe4}
...
Nun wurden beide Komponenten in der Mitte der Spulen senkrecht zur Achse gemessen. Die zur Radialkomponente gemessene Spannung lag jeweils deutlich unter $\SI{1}{mV}$, was wir auf leichte Stellungsfehler der Testspule zurückführen (siehe Diskussion).
\begin{figure}[H]
\centering
\begin{tikzpicture}
  \begin{axis}[
    ...
    xmin=-5, xmax=235,
    ymin=-0.5, ymax=4.75,
    xlabel={Abstand zur Spulenachse $x$ [\si{mm}]},
    ylabel={$B_\parallel$ [\si{mT}]},
    domain=-12:160,
    legend entries={Messwerte},
    legend pos=north east
  ]
  \addplot+ plot [only marks,mark=x, error bars/.cd, x dir=both, x fixed=1, y dir=both, y fixed=0.002]  table {helmaxialsenkrecht.csv};
  \end{axis}
\end{tikzpicture}
...
\subsection{Magnetfeld in horizontaler Symmetrieebene eines Quadrupols}
\begin{figure}[H]
  \centering
\input{aufgabe9}
\caption{Feldvektoren des $\vec{B}$-Feldes bei einem Quadrupol}
\end{figure}
\section{Diskussion}
